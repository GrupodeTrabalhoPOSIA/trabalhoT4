...
import { useConfiguredModel } from '@/common/modelPolicy';
...
function AppShell({ children, activePage }: AppShellProps) {
  const model = useConfiguredModel();
  ...
  return (
    <div className="app-shell">
      ...
      <div className="portal-utility-bar"><div className="portal-model-policy">Modelo do projeto: {model ? <a href={`https://openrouter.ai/${model.split('/').map(encodeURIComponent).join('/')}`} target="_blank" rel="noreferrer">{model}</a> : <span>Configuração indisponível</span>}</div><nav aria-label="Recursos complementares"><span>Laboratório</span><a href={pagePaths.chat} aria-current={activePage === 'chat' ? 'page' : undefined}>{pageTitles.chat}</a><a href={pagePaths.knowledge} aria-current={activePage === 'knowledge' ? 'page' : undefined}>{pageTitles.knowledge}</a>{interactive && <LiveApiStatus />}</nav></div>
