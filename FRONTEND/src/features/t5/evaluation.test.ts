...
describe('Protocolo T5', () => {
  ...
  it('preserva falhas da primeira tentativa e exclui livres e simulações da latência', () => {
    ...
    expect(metrics([first, retry, free, simulation])).toMatchObject({ count: 2, reviewed: 2, success: 50, p95: 45 });
  ...
  it('não inventa métricas e não aceita outra candidata/modelo', () => {
    ...
    expect(metrics([makeRun({ model: 'outro' }), makeRun({ candidate: 't5-v0' })]).count).toBe(0);
  });
  it('exige pessoas distintas para a mesma execução do conjunto humano', () => {
    ...
    expect(pairedReviews([makeRun()], [review, { ...review, evaluator: ' pessoa a ' }])).toBe(0);
    expect(pairedReviews([makeRun()], [review, { ...review, evaluator: 'Pessoa B' }])).toBe(1);
    expect(pairedReviews([makeRun()], [review, { ...review, execution_id: 'outra', evaluator: 'Pessoa B' }])).toBe(0);
  });
