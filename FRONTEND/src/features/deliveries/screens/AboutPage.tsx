...
export default function AboutPage() {
  return <article className="delivery-page about-page">
    ...
    <section className="about-purpose" aria-labelledby="about-purpose-heading"><div><span className="eyebrow">O trabalho</span><h2 id="about-purpose-heading">Da experimentação<br />à aplicação com LLMs.</h2></div><div><p>O projeto investiga como modelos de linguagem e técnicas de engenharia de prompt podem apoiar consultas à política de trabalho híbrido da Aurora Tech. O copiloto de RH auxilia colaboradores e gestores a entender regras, verificar elegibilidade e obter orientações de segurança a partir de uma base autorizada.</p><p>As entregas documentam essa evolução: comparação de modelos e prompts (T1), especificação e baseline (T2), biblioteca de prompts versionados (T3), integração em um fluxo textual (T4) protótipo multimodal com plano de avaliação (T5) e consolidação da entrega final (T6).</p><p>O portal reúne os relatórios, as decisões, os resultados e as pendências para facilitar a leitura e a correção do professor. O frontend apresenta as evidências e o backend executa os fluxos dos Trabalhos 4, 5 e 6.</p></div></section>
    <aside className="delivery-notice"><strong>Finalidade acadêmica e limites do protótipo</strong><p>Os modelos de geração e embeddings são definidos no ambiente do backend. Embeddings são usados na busca de documentos; o modelo de geração produz as respostas.</p><p>Esta aplicação é uma demonstração acadêmica, não um canal oficial de atendimento de RH. Não aprova exceções nem toma decisões individuais. Resultados históricos e simulações são identificados; a avaliação factual e a aprovação das evidências exigem revisão humana.</p></aside>
    <div className="about-actions"><a className="delivery-button delivery-button--primary" href={pagePaths.overview}>Conhecer as entregas →</a><a className="delivery-button" href={pagePaths.t6}>Abrir a entrega final ↗</a></div>
    <AcademicFooter />
  </article>;
}
