...
export default function OverviewPage() {
  return <div className="delivery-page">
    <header className="portal-hero">
      <div><span className="eyebrow">Portfólio acadêmico · Entregas 01—04</span><h1>Um copiloto.<br /><em>Uma construção em etapas.</em></h1><p>Da escolha do modelo ao fluxo executável: acompanhe as decisões, os resultados e as evidências do copiloto de RH da Aurora Tech.</p><a className="delivery-button delivery-button--primary" href={pagePaths.t4}>Experimentar o Trabalho 4 <span aria-hidden="true">↗</span></a></div>
      <aside className="portal-guide" aria-labelledby="guide-title"><span className="eyebrow">Para quem avalia</span><h2 id="guide-title">O percurso está aqui.</h2><ol><li><strong>Entenda a decisão</strong><span>Objetivo e método de cada trabalho.</span></li><li><strong>Confira a evidência</strong><span>Resultados históricos e relatórios originais.</span></li><li><strong>Inspecione o protótipo</strong><span>Execute o T4 e exporte seus registros.</span></li></ol></aside>
    </header>
    <section aria-labelledby="deliveries-heading"><div className="delivery-section-heading"><div><span className="eyebrow">Linha de evolução</span><h2 id="deliveries-heading">As entregas do projeto</h2></div><p>Cada etapa deixa uma base para a próxima.</p></div>
      <div className="delivery-cards">{deliveries.map(delivery => <a className={`delivery-card${delivery.id === 't4' ? ' delivery-card--current' : ''}`} href={pagePaths[delivery.id]} key={delivery.id} aria-label={`Abrir Trabalho ${Number(delivery.number)}: ${delivery.title}`}>
    ...
    <aside className="delivery-notice"><strong>Transparência sobre as evidências</strong><p>As páginas T1–T3 sintetizam os relatórios fornecidos pelo grupo, sem reexecutar os experimentos. Pendências continuam identificadas. O T4 é interativo e sua validação final depende das execuções, da revisão humana e da exportação dos resultados.</p></aside>
    <AcademicFooter />
  </div>;
}
