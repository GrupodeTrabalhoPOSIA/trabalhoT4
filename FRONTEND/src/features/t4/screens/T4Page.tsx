...
export default function T4Page() {
  ...
  const realModelReady = config?.model === PROJECT_MODEL_ID;
  ...
  return <div className="t4-page">
    ...
    <p className="t4-muted">Modelo de geração do projeto: <strong>Mistral Large ({PROJECT_MODEL_ID})</strong>, escolhido no T1 e mantido nas entregas seguintes. O campo acima mostra a configuração informada pela API, sem substituí-la por um rótulo fixo.</p>
    {config && !realModelReady && <p className="t4-notice" role="alert">O backend informa um modelo diferente do Mistral Large. Execuções reais estão bloqueadas para preservar a escolha do T1. Atualize o backend e recarregue esta página; simulações continuam disponíveis.</p>}
    ...
    {tab === 'prototype' && <div className="t4-workspace"><div className="t4-main-column">
      <section className="t4-input-panel" aria-labelledby="question-heading"><div className="t4-panel-heading"><span className="eyebrow">Entrada do fluxo</span><h2 id="question-heading">Experimente uma pergunta</h2><p className="t4-muted">Explique uma regra, verifique elegibilidade ou peça orientação de segurança.</p></div>
        ...
        <form onSubmit={e => { e.preventDefault(); void execute(question.trim(), correction.trim(), testCase); }}>
          ...
          <p className="t4-input-hint">{simulation ? 'Saídas de teste serão identificadas como simuladas na exportação.' : 'Usa Mistral Large após confirmação da configuração do backend e pode consumir créditos OpenRouter.'}</p>
        </form>
      </section>
    ...
    {tab === 'evidence' && <EvidencePanel runs={runs} busy={busy} onLoad={loadCase} onInspect={id => { setSelectedId(id); setTab('prototype'); }} />}
