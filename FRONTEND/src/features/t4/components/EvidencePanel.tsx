...
export default function EvidencePanel({ runs, busy, onLoad, onInspect }: { runs: Evidence[]; busy: boolean; onLoad: (testCase: TestCase) => void; onInspect: (id: string) => void }) {
  ...
  return <section className="t4-evidence" aria-labelledby="evidence-heading">
    ...
    <p className="t4-muted">Referência T2 e modelo mantido no T4: Mistral Large (mistralai/mistral-large). Se houver uma primeira execução real com outro modelo, a comparação é sinalizada como divergente; o registro original é preservado na exportação. Tokens e custos não são disponibilizados pelo cliente atual e não são estimados.</p>
